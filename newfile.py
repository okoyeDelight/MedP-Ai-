...
if not st.session_state.logged_in_user:
    ...
    auth_mode = st.sidebar.radio("Choose Action", ["Login", "Sign Up"], horizontal=True)
    auth_user = st.sidebar.text_input("Nickname")
        ...
        if auth_user and auth_pass:
            if auth_mode == "Sign Up":
                if auth_user in users_db:
                    st.sidebar.error("Nickname taken! Choose another or log in.")
                else:
                    ...
                    st.toast(f"Account created! Welcome {auth_user} 🎉", icon="✅")
                    st.rerun()
            elif auth_mode == "Login":
                if auth_user in users_db and users_db[auth_user]["password"] == auth_pass:
                    st.session_state.logged_in_user = auth_user
                    st.toast(f"Welcome back, {auth_user}! 🚀", icon="✅")
                    st.rerun()
                else:
                    st.sidebar.error("Invalid Nickname or Password.")
        else:
            st.sidebar.warning("Please fill in both fields.")
    st.warning("🔒 Please Login or Sign Up in the sidebar to access Desprix Med AI features.")
    st.stop()

# --- USER IS LOGGED IN past this point ---
username = st.session_state.logged_in_user
user_data = users_db[username]
# Ensure old accounts have the new lists
...
pic_upload = st.sidebar.file_uploader("Change Profile Picture", type=['png', 'jpg', 'jpeg'], label_visibility="collapsed")
if pic_upload:
...
st.sidebar.markdown(f"<div style='text-align: center;'><strong>Total XP:</strong> {user_data['score']} ⭐️</div><br>", unsafe_allow_html=True)
